...
// Let T(n) be the time complexity of bubble sort.
// In each recursive call, we perform n-1 comparisons and swaps.
// Therefore, T(n) = T(n-1) + (n-1)
// Solving this gives us T(n) = O(n^2).

// b) Substitution Method:
// Assume T(n) = c*n^2 for some constant c.
// Then T(n) = T(n-1) + (n-1) = c*(n-1)^2 + (n-1)
// Expanding and simplifying leads to T(n) = O(n^2).